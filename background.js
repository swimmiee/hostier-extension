// Hostier Chrome Extension — Background Service Worker
...
/**
 * Hostier 세션 쿠키를 읽어서 Cookie 헤더로 포함하여 fetch.
 */
async function fetchWithSession(url, options = {}) {
  const cookies = await chrome.cookies.getAll({ url: HOSTIER_URL });
...
async function refreshSessionCookie(platform) {
  ...
  try {
    ...
    if (freshCookie && freshCookie.value !== currentCookie.value) {
      console.log(`[hostier] ${platform} session refreshed (cookie changed)`);
    } else {
      console.log(
        `[hostier] ${platform} session refresh: no cookie change (status=${res.status})`,
      );
    }
  } catch (e) {
    console.log(`[hostier] ${platform} session refresh failed:`, e.message || e);
  }
}
...
async function captureAndSendToken(platform) {
  ...
  try {
    ...
    if (!cookie || !cookie.value) {
      console.log(`[hostier] No ${config.name} cookie found for ${platform}`);
      return;
    }
    ...
    const res = await fetchWithSession(
      `${HOSTIER_URL}/api/platform-connections`,
      {
        method: "POST",
    ...
    if (res.ok) {
      console.log(`[hostier] ${platform} token synced`);
      ...
      console.warn(
        `[hostier] ${platform} token sync failed: ${res.status}`,
        errorBody,
      );
    ...
    console.error(`[hostier] Failed to send ${platform} token:`, e);
...
async function syncAllTokens() {
  console.log("[hostier] Periodic token sync started");

  const connected = await getConnectedPlatforms();
  if (connected.size === 0) {
    console.log("[hostier] No connected platforms, skipping sync");
    return;
  }
  ...
  for (const platform of connected) {
    if (MANUAL_RECONNECT_ONLY_PLATFORMS.has(platform)) {
      console.log(
        `[hostier] Skipping periodic token sync for ${platform} (manual reconnect only)`,
      );
      continue;
    }
    await captureAndSendToken(platform);
  }
  console.log("[hostier] Periodic token sync complete");
}
...
chrome.cookies.onChanged.addListener(async (changeInfo) => {
  ...
  for (const [platform, config] of Object.entries(PLATFORM_COOKIES)) {
    ...
    if (cookie.name === config.name && hostname.endsWith(cookieDomain)) {
      ...
      if (isPending) {
        console.log(
          `[hostier] Detected ${platform} cookie after login — connecting`,
        );
        await captureAndSendToken(platform);
        ...
        if (
          ...
          console.log(`[hostier] Detected ${platform} cookie change — syncing`);
          captureAndSendToken(platform);
        } else if (connected.has(platform)) {
          console.log(
            `[hostier] Detected ${platform} cookie change — ignoring automatic sync (manual reconnect only)`,
          );
        }
      }
      break;
...
chrome.runtime.onInstalled.addListener(() => {
  console.log(
    "[hostier] Extension installed/updated — syncing connected platforms",
  );
  syncAllTokens();
});
